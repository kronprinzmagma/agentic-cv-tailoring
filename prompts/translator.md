...
## Abschnittsüberschriften (hart)

Die Überschriften der drei generierten Abschnitte sind ein Vertrag mit dem
PDF-Renderer, keine Stilfrage. Übersetze sie **exakt** so:
...
Nicht "Core Competencies", nicht "Core Skills", nicht "Professional Background".
Alle drei bleiben auf Ebene `##`. Eine Überschrift auf `#` lässt den ganzen
Abschnitt aus dem PDF fallen.

## Stations-Header (hart)

Firmennamen in `### Jahr | Firma – Titel`-Headern bleiben unverändert (HealthApp,
MediaCorp, MediaHoldingCo, GastroSaaS / local-directory.example, Namics AG).
Die zwei Stationen ohne Firma haben **feste** englische Header — die
Endprüfung gleicht die Stationen DE↔EN ab und verwirft die Übersetzung, wenn
eine Station nicht mehr erkennbar ist:

| Deutsch | Englisch |
|---|---|
| `### 2026–heute \| Eigenregie – KI-Produktentwicklung` | `### 2026–present \| Independent – AI Product Development` |
| `### 2025 \| Familiär bedingte Auszeit` | `### 2025 \| Family-related Career Break` |

`heute` wird in Zeiträumen immer zu `present`. Das ` – ` zwischen Firmen- und
Titelteil bleibt erhalten (das Template braucht den Trenner).
