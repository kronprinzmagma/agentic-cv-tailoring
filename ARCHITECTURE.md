...
Four deterministic checks run during the pipeline to stop expensive LLM
...
A nice-to-have marker ("von Vorteil", "nice to have", "plus") in either the
analyst's comment **or the requirement text itself** downgrades a gap from
critical to soft.
...
**Gap-to-question routing.** Gaps don't just gate — they become questions.
`format_gaps_as_questions()` turns LÜCKE / qualified-SCHWACH rows into
clarification questions that the initial factcheck merges into
`02_klaerungsfragen.md` (critical first, capped at 6, with an explicit
"I don't have this" opt-out). Rationale: the standard CV and references are
a lossy compression of a whole career — many "gaps" are answerable, and the
pipeline should ask before it transfers adjacent experience or silently
drops the topic. Questions fire only in the first factcheck round (once
answers exist, re-asking would create an endless pause loop), and
requirements already covered by a stored clarification answer are
conservatively suppressed via word-overlap matching.
...
- **Bullet rule:** Every bullet in the Work Experience section must be
  ...
  listed.
- **Summary rule:** The Management Summary targets 120–160 words. Vetoes
  fire at >170 words (cut whole sentences) and <110 words (usually a
  missing differentiation paragraph, not missing filler).
- **Competency-count rule:** The Key Competencies section is capped at
  6 bold-headline entries — a 7th entry has historically pushed the
  rendered PDF past the 3-page limit.
...
### Cross-Section Redundancy Check (`src/cv_tailor/redundancy_check.py`)

Runs once on the assembled final CV, after the writer loop. Reviewers only
ever see one section at a time, so verbatim flagship-claim repetition
between the Management Summary and the Work Experience section is invisible
to them. This check finds shared word runs of ≥8 words between the Summary
and every other section.

**Report-only by design:** findings go to `_redundancy.md` (CLI hint +
Web UI tab), the CV is never rewritten automatically. Repetition is a
judgement call — sometimes a fact legitimately belongs in two places — so
the operator decides. A second, preventive layer feeds the accepted
Management Summary into the Writer's context for later sections with a
no-verbatim-repeat rule (best-effort: skipped when sections run
concurrently).
...
- **Dependency security.** Transitive dependencies are pinned in
  `pyproject.toml` at their patched minimum versions (`urllib3>=2.7.0`,
  `aiohttp>=3.14.0`, `idna>=3.15`) to resolve known CVEs.
