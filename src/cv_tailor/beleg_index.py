...
class RawClaim:
    ...
    quelle_typ: str        # "standard_cv" | "zeugnis" | "portfolio"
...
# Portfolio sections that carry evidence. Everything else in PORTFOLIO.md is
# meta (header block, update convention, changelog) and must not become a
# Beleg — a changelog line is a statement about the document, not about Alex.
PORTFOLIO_SECTIONS = (
    "Formale Weiterbildung",
    "Berufliche KI-Vorerfahrung",
    "Eigene Projekte",
    "Querschnitt",
    "Wachstumskanten",
)
...
def parse_standard_cv(path: Path) -> list[RawClaim]:
    """Parse the Standard-CV Markdown into deterministic RawClaim records.

    Thin wrapper over `parse_markdown_source` — kept as the public entry point
    because callers and tests reference it by name.
    """
    if not path.exists():
        raise FileNotFoundError(
            f"Standard-CV not found at {path}. Expected location: data/standard_cv.md"
        )
    return parse_markdown_source(path, source_typ="standard_cv")


def parse_portfolio(path: Path) -> list[RawClaim]:
    """Parse PORTFOLIO.md into RawClaim records, evidence sections only.

    The portfolio is a living document maintained outside this repo (see
    `sources.portfolio` in config.yaml). Only PORTFOLIO_SECTIONS are indexed;
    header block, update convention and changelog describe the *document*,
    not Alex, and would pollute the index.

    Returns an empty list when the file is absent — the portfolio is optional.
    """
    if not path.exists():
        log.warning("parse_portfolio.missing", path=str(path))
        return []
    return parse_markdown_source(
        path, source_typ="portfolio", top_sections=PORTFOLIO_SECTIONS
    )
...
def parse_markdown_source(
    ...
    """Parse a Markdown file into deterministic RawClaim records.

    Args:
        path: Path to the Markdown file.
        source_typ: Provenance tag stored on every claim.
        top_sections: When given, only content under an H1/H2 heading whose
            text starts with one of these is emitted. `None` takes everything.
    ...
    for idx, raw_line in enumerate(lines):
        ...
        # Section gate: skip anything outside the requested top-level sections.
        if top_sections is not None and not (
            current_top and current_top.startswith(top_sections)
        ):
            continue
...
class BelegEntry:
    ...
    typ: str
    quelle_typ: str
    # Source heading this claim sits under. Optional for backwards
    # compatibility (pre-existing entries carry none); the portfolio needs it
    # so "Wachstumskanten" can be told apart from evidence — a gap rendered
    # as an activated Beleg is exactly the drift this project guards against.
...
def load_portfolio_path(config_path: Path = Path("config.yaml")) -> Path | None:
    """Return the configured portfolio path, or None when not configured."""
    if not config_path.exists():
        return None
    cfg = yaml.safe_load(config_path.read_text(encoding="utf-8")) or {}
    raw = (cfg.get("sources") or {}).get("portfolio")
    return Path(raw).expanduser() if raw else None


def enrich_claims(
    claims: Iterable[RawClaim],
    fc_config: dict,
    start_id: int = 1,
) -> list[BelegEntry]:
    """Classify claims and wrap them as BelegEntry.

    `start_id` lets callers append to an existing index without renumbering:
    BELG-IDs are referenced from clarifications, prompts and review notes, so
    a full rebuild would silently invalidate every one of them.
    """
    entries: list[BelegEntry] = []
    for idx, claim in enumerate(claims, start=start_id):
...
def format_beleg_index_compact(index: dict, include_portfolio: bool = False) -> str:
    ...
    Portfolio entries are excluded by default: they more than double the
    prefix (~18k extra tokens), which alone exceeds the Anthropic Tier 1
    budget of 30k input tokens per minute. They reach analyst and writer
    through the job-filtered Experience Activation Map instead, which is
    where role-dependent evidence belongs.
    ...
    for e in index.get("entries", []):
        if not include_portfolio and e.get("quelle_typ") == "portfolio":
            continue
