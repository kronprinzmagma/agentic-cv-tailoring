...
def run_page_trim_if_needed(
    ctx: RunContext,
    progress_cb: ProgressCb = None,
    max_trim_rounds: int = 2,
) -> int:
    """Check page count of 04_final_de.md; if > 3 run trim passes until it fits.

    Returns the final page count (-1 if Playwright unavailable / template missing).
    Skips silently if CV_TAILOR_SKIP_PDF=1.
    """
    import os
    if os.environ.get("CV_TAILOR_SKIP_PDF") == "1":
        return -1

    from cv_tailor.pdf_renderer import MAX_PAGES, count_pages_de

    pages = count_pages_de(ctx)
    if pages <= 0:
        # Playwright not available or template missing — skip silently
        return pages

    if pages <= MAX_PAGES:
        log.info("pipeline_stages.page_check_ok", run_id=ctx.run_id, pages=pages)
        return pages

    _emit(progress_cb, f"⚠ CV hat {pages} Seiten — Trim-Pass läuft …")
    log.warning("pipeline_stages.page_overflow_before_trim", run_id=ctx.run_id, pages=pages)

    from cv_tailor.agents.writer_trim import run_writer_trim

    for attempt in range(1, max_trim_rounds + 1):
        run_writer_trim(ctx, pages=pages)
        pages = count_pages_de(ctx)
        if pages <= MAX_PAGES:
            _emit(progress_cb, f"✓ CV nach Trim-Pass {attempt}: {pages} Seiten")
            log.info("pipeline_stages.trim_ok", run_id=ctx.run_id, pages=pages, attempt=attempt)
            return pages
        _emit(progress_cb, f"↻ Noch {pages} Seiten nach Trim-Pass {attempt} — weiterer Versuch …")

    _emit(progress_cb, f"⚠ CV hat nach {max_trim_rounds} Trim-Pässen noch {pages} Seiten")
    log.warning("pipeline_stages.trim_exhausted", run_id=ctx.run_id, pages=pages, attempts=max_trim_rounds)
    return pages
...
def run_translator_stage(ctx: RunContext, progress_cb: ProgressCb = None) -> Path | None:
    ...
    if translated_path is not None:
        ...
        _run_idiom_check(ctx, translated_path, progress_cb)
    else:
        check_md = ctx.run_dir / "_translation_check.md"
        if check_md.exists() and "VERWORFEN" in check_md.read_text(encoding="utf-8"):
            _emit(
                progress_cb,
                "⚠ Übersetzung VERWORFEN — Endprüfung fehlgeschlagen "
                "(siehe _translation_check.md). DE-CV bleibt das Versand-Artefakt.",
            )
        else:
...
def _run_idiom_check(
    ctx: RunContext,
    translated_path: Path,
    progress_cb: ProgressCb = None,
) -> None:
    """Scan the English CV for German calques; report-only, never fatal.

    Runs here rather than in `writer_loop` (where `redundancy_check` lives)
    because the English text only exists once the translator has run.
    """
    from cv_tailor.idiom_check import write_idiom_report

    try:
        report = write_idiom_report(ctx.run_dir, translated_path.read_text(encoding="utf-8"))
    except Exception as exc:  # noqa: BLE001
        write_run_log_entry(ctx.run_dir, "idiom_check", f"Idiom-Check fehlgeschlagen: {exc}")
        return
    if report is not None:
        _emit(progress_cb, f"  ↳ Englisch-Hinweise: {report.name}")
        write_run_log_entry(
            ctx.run_dir, "idiom_check",
            "Mögliche Germanismen im englischen CV gefunden — _idiom.md geschrieben",
        )
