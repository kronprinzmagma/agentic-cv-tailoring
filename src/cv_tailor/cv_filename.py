...
# Umlaut / special-char transliteration table for upload-safe filenames
...
# Maximum lengths for company and position tokens (characters, after stripping)
_MAX_COMPANY_LEN = 25
_MAX_POSITION_LEN = 30

# Legal entity suffixes to strip from company names
# `# Stellenanzeige — Firma: Position` — the Jobcoach's heading form. The
# company may itself contain a colon-free dash ("Swisscom (Schweiz) AG"), so
# the split is on the first colon after the em/en-dash.
_COACH_HEADING = re.compile(
    r"^(?:stellenanzeige|stelleninserat|job\s*posting)\s*[—–\-]\s*([^:]+?)\s*:\s*(.+)$",
    flags=re.IGNORECASE,
)
...
# "100%", "80-100%", "80–100 %", "(80–100 %)" — the first number may carry
# no percent sign of its own.
_PERCENTAGES = re.compile(r"\s*\(?\s*\d+\s*%?(?:\s*[-–]\s*\d+)?\s*%\s*\)?")
...
def _load_output_config(config_path: Path = _CONFIG_PATH) -> tuple[str, str]:
    ...
    try:
        ...
        # Defensiv: config.yaml konnte nicht geladen werden — Defaults greifen.
        # Sichtbar machen statt still verschlucken.
        log.warning(
            "cv_filename.config_load_failed",
...
def _sanitize(value: str) -> str:
    ...
    - Transliterates umlauts and special chars (ä→ae, —→'', etc.)
    - Replaces path-illegal characters with a space
    - Collapses whitespace runs
    - Strips leading/trailing whitespace
    """
    cleaned = value.translate(_UMLAUT_TABLE)
    cleaned = _BAD_FS_CHARS.sub(" ", cleaned)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
    return cleaned


def _truncate_token(value: str, max_len: int) -> str:
    """Truncate `value` to `max_len` characters without cutting mid-word."""
...
def _extract_company(posting_text: str) -> str | None:
    ...
    Strips legal entity suffixes (AG, GmbH, Ltd, …) and secondary info
    (anything after the first comma or dash) for a short, clean filename token.
    """
    m = re.search(
        r"\*\*(?:Arbeitgeber|Unternehmen|Firma):\*\*\s*(.+?)(?:\n|$)", posting_text
    )
    if m:
        name = m.group(1).strip()
    else:
        # Jobcoach header convention (since 2026-09):
        # `# Stellenanzeige — Firma: Position`. The last three runs had to be
        # renamed by hand because neither branch above matched.
        m = _COACH_HEADING.match(_first_h1(posting_text) or "")
        if not m:
            return None
        name = m.group(1).strip()
    # Klammern tragen Quellen- und Metadaten-Hinweise ("Sunrise GmbH (in den
    # Seiten-Metadaten der Anzeige: Sunrise Communications AG)"). Vor 2026-09-07
    # blieb deren Inhalt stehen und landete als "Sunrise GmbH in den" im
    # Dateinamen, den der Recruiter sieht. Alles ab der ersten Klammer weg.
    name = re.split(r"[(\[]", name, maxsplit=1)[0].strip()
    # Take only the primary part before comma, em/en-dash or middot separator
    name = re.split(r",|—|–|·", name, maxsplit=1)[0].strip()
    name = _LEGAL_SUFFIXES.sub("", name).strip()
    name = _sanitize(name)
    name = _truncate_token(name, _MAX_COMPANY_LEN)
    return name or None


def _first_h1(posting_text: str) -> str | None:
...
def _extract_position(posting_text: str) -> str | None:
    ...
    Strips:
    - "Stellenanzeige:"-style prefixes, incl. the Jobcoach form
      `Stellenanzeige — Firma: Position`
    ...
    for line in posting_text.splitlines():
        ...
        if line.startswith("# ") and not line.startswith("## "):
            ...
            coach = _COACH_HEADING.match(heading)
            if coach:
                heading = coach.group(2).strip()
            ...
            # Drop comma-separated secondary info (e.g. ", digitale Plattformen…")
            ...
            # Drop trailing single-letter suffixes like "(a)", "(b)"
            heading = re.sub(r"\s*\([a-z]\)\s*$", "", heading, flags=re.IGNORECASE)
            heading = _sanitize(heading)
            heading = _truncate_token(heading, _MAX_POSITION_LEN)
