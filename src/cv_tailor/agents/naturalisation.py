...
def _extract_json(text: str) -> dict:
    """Parse the first complete JSON object from the model response.

    Tolerates code fences, prose before the object and — the reason for
    `raw_decode` — trailing content after it. The previous
    `find("{") … rfind("}")` slice swallowed every later brace, so a model
    that appended a closing remark or a second block produced
    "Extra data: line 133 column 1" and, one layer up, a silently empty
    suggestion list (2026-08-25, SBB-Lauf).
    """
    s = text.strip()
    if s.startswith("```"):
        s = s.strip("`").strip()
...
def run_naturalisation(
    ...
    # The posting travels along as a *vocabulary* reference so the agent can
    # align wording with industry-standard terminology. The prompt forbids
    # importing claims or mirroring keywords the CV has no basis for.
    posting_path = ctx.run_dir / "00_stellenanzeige.md"
    posting_block = ""
    if posting_path.exists():
        posting_text = posting_path.read_text(encoding="utf-8")[:6000]
        posting_block = (
            "\n\n## Stellenanzeige (nur Vokabular-Referenz, keine Inhalte übernehmen)\n"
            f"{posting_text}"
        )
    user_msg = f"## Sprache\n{lang}\n\n## CV-Markdown\n{md_text}{posting_block}"
    ...
    for s in raw_suggestions:
        if not isinstance(s, dict):
            # Count it — a silent `continue` here made a 2'700-token answer
            # look like "nothing to improve" (2026-09-19, FIFA run).
            dropped.append({"id": "", "reason": f"not an object: {type(s).__name__}"})
            continue
        ...
        if original not in md_text:
            ...
            continue
        # Suggestion may not exceed original by more than 20% — naturalisation
        # trims, it does not embellish. Exception: `germanism` fixes replace a
        # calque with idiomatic English, which often *needs* the articles and
        # possessives German drops ("decision basis for X" → "used them to
        # inform X"). Capping those at 120% silently dropped the good ones.
        max_ratio = 1.45 if kategorie.strip().lower() == "germanism" else 1.2
        if len(vorschlag) > int(len(original) * max_ratio):
            dropped.append({
                "id": sid,
                "reason": f"vorschlag exceeds {int(max_ratio * 100)}% of original",
            })
            continue
        seen_ids.add(sid)
        validated.append({
