...
def run_translator(
    ...
    # Final gate: the translation runs AFTER factcheck/consistency and was
    # previously shipped unvalidated. Deterministic check — fatal findings
    # (empty/truncated output, station set differs from DE) reject the EN
    # artifact entirely; the validated DE CV remains the shipped output.
    # Number drift DE↔EN is report-only (_translation_check.md).
    from cv_tailor.final_check import format_translation_report, validate_translation

    check = validate_translation(final_cv_text, content)
    report_path = ctx.run_dir / "_translation_check.md"
    if check.fatal:
        report_path.write_text(format_translation_report(check), encoding="utf-8")
        # Keep the raw output for inspection — a rejected translation is
        # still a paid LLM call and the fastest way to see *why* the gate
        # fired. Never named 04_final_en.md, so nothing downstream ships it.
        (ctx.run_dir / "_rejected_en.md").write_text(content + "\n", encoding="utf-8")
        write_run_log_entry(
            ctx.run_dir,
            "translator",
            "Übersetzung VERWORFEN — deterministische Endprüfung fehlgeschlagen: "
            + " | ".join(check.fatal),
        )
        log.error(
            "translator.validation_failed",
            run_id=ctx.run_id,
            fatal=check.fatal,
    ...
    if check.warnings:
        ...
        write_run_log_entry(
            ctx.run_dir,
            "translator",
            f"Zahlen-Abgleich DE↔EN: {len(check.warnings)} Hinweis(e) — _translation_check.md",
        )
    elif report_path.exists():
        # Clean re-run — remove a stale report from a previous attempt
        report_path.unlink()
