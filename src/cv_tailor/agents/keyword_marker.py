...
def _bold_phrase_in_text(text: str, phrase: str, used: set[str]) -> tuple[str, bool]:
    ...
    if phrase in used:
        return text, False
    # Suche den ersten geeigneten Treffer. Treffer werden uebersprungen, wenn sie
    # in einer Header-Zeile liegen, an bestehende **-Marker grenzen oder mitten in
    # einem Wort sitzen (Wortgrenzen-Pruefung gegen z.B. **Lead**ership).
    search_from = 0
    while True:
        ...
        # Wortgrenzen-Pruefung: Zeichen direkt vor und nach dem Treffer duerfen
        # nicht alphanumerisch sein, sonst faengt der Treffer wort-intern.
        char_before = text[idx - 1] if idx > 0 else ""
        char_after = text[end] if end < len(text) else ""
        if char_before.isalnum() or char_after.isalnum():
            search_from = idx + 1
            continue
        # Treffer nicht innerhalb einer Header-Zeile
        ...
        if text[line_start:line_end].lstrip().startswith("#"):
            search_from = idx + 1
            continue
        # Treffer nicht bereits innerhalb von **-Markern
        before_2 = text[max(0, idx - 2):idx]
        after_2 = text[end:end + 2]
        if before_2.endswith("**") or after_2.startswith("**"):
            search_from = idx + 1
            continue
        result = text[:idx] + f"**{phrase}**" + text[end:]
        used.add(phrase)
        return result, True
