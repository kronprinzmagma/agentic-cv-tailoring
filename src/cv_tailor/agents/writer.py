...
def load_style_exemplars(
    ...
    for fp in files:
        ...
        # alphabetic run in the stem against the posting slug; the author
        # tokens are filtered out below, so the stem needs no pre-stripping.
...
def run_writer(
    ...
    # Cross-section dedup context: sections run sequentially by default
    # (writer_section_concurrency: 1), so when schluesselkompetenzen or
    # berufserfahrung starts, the accepted Management Summary already lies
    # in 03_iterationen/. Feed it to the writer so flagship claims aren't
    # repeated verbatim across sections (writer.md "Wiederholungs-Regel").
    # Best-effort: with concurrency > 1 the file may not exist yet — skip.
    summary_context_text = ""
    if section != "management_summary":
        iter_dir = ctx.run_dir / "03_iterationen"
        summary_versions = sorted(iter_dir.glob("management_summary_v*.md")) if iter_dir.exists() else []
        summary_versions = [p for p in summary_versions if re.fullmatch(r"management_summary_v\d+\.md", p.name)]
        if summary_versions:
            summary_context_text = summary_versions[-1].read_text(encoding="utf-8")
    ...
    if summary_context_text:
        dynamic_parts.append(
            "## Bereits geschriebene Management Summary (nicht wortgleich wiederholen)\n"
            + summary_context_text
        )
    if prev_text:
