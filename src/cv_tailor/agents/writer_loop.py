...
def _process_section(
    ...
    for round_num in range(1, MAX_ROUNDS + 1):
        ...
        if section == "berufserfahrung":
            consistent, issues = validate_berufserfahrung(
                draft_text, STANDARD_CV_PATH, check_completeness=True
            )
        ...
        if (
            ...
            # Order + bare-header fallback (rules 5/6). Runs on the header-
            # normalised text so the block splitter sees canonical headers.
            fixed_text, layout_fixes = autofix_station_layout(fixed_text, STANDARD_CV_PATH)
            fixes_applied = fixes_applied + layout_fixes
            if fixes_applied:
                ...
                write_run_log_entry(
                    ctx.run_dir,
                    "consistency_autofix",
                    f"{section}: {len(fixes_applied)} Header/Layout-Korrektur(en) automatisch angewendet",
                )
