...
# The sections the writer/translator produce. The template renders `## `
# blocks only — a heading that arrives as `# ` is dropped together with its
# body, silently and without changing the page count. Seen 2026-08-24: the
# translator omitted its own `# Final CV (EN)` title line and emitted
# `# Management Summary` instead, costing the whole summary in the PDF.
_GENERATED_SECTION_HEADINGS = (
    "management summary",
    "schlüsselkompetenzen", "key competencies", "core competencies",
    "berufserfahrung", "professional experience",
)

# Groups of interchangeable DE/EN headings — one member of each group must
# survive into the stitched markdown, or the PDF would ship incomplete.
_REQUIRED_SECTION_GROUPS = (
    ("Management Summary",),
    # Der Translator schwankt zwischen "Key" und "Core Competencies" (historisch
    # 15x Core gegen 10x Key). Beide sind gutes Englisch; blockiert wurde bisher
    # nur Core. prompts/translator.md pinnt "Key Competencies" als Vertrag,
    # dieser Guard akzeptiert defensiv beide.
    ("Schlüsselkompetenzen", "Key Competencies", "Core Competencies"),
    ("Berufserfahrung", "Professional Experience"),
)
...
def _normalise_section_headings(text: str) -> str:
    """Promote generated section headings from `# ` to `## `.

    Only the known section names are touched — a genuine title line
    (`# Finaler CV (DE)`) keeps its level so the meta-header strip still
    matches it.
    """
    def promote(match: re.Match) -> str:
        title = match.group("title").strip()
        if title.lower() in _GENERATED_SECTION_HEADINGS:
            return f"## {title}"
        return match.group(0)

    return _H1_RE.sub(promote, text)


def missing_generated_sections(stitched_md: str) -> list[str]:
    """Required generated sections absent from the stitched template markdown.

    Returns the first name of each missing group, empty list when complete.
    """
    headings = {
        m.group(1).strip().lower()
        for m in re.finditer(r"^##\s+(.+?)\s*#*\s*$", stitched_md, flags=re.M)
    }
    return [
        group[0] for group in _REQUIRED_SECTION_GROUPS
        if not any(name.lower() in headings for name in group)
    ]
...
def build_template_markdown(
    ...
    # Lift section headings to `## ` first — the meta-header strip below uses
    # a `^##` lookahead, and a summary sitting at `# ` would otherwise be
    # both unfindable for the template and invisible to that lookahead.
    generated = _normalise_section_headings(generated)
...
def run_pdf_renderer(
    ...
    for lang in ("de", "en"):
        ...
        # A section that never made it into the stitched markdown would ship
        # as a plausible-looking PDF with content missing. Same contract as
        # the overflow guard: marker file, no PDF, canonical MD stays valid.
        missing = missing_generated_sections(md)
        if missing:
            marker = ctx.run_dir / f"_pdf_missing_sections_{lang}.md"
            marker.write_text(
                "# Fehlende Abschnitte im PDF-Markdown\n\n"
                f"Beim Zusammenbau des {lang.upper()}-Templates fehlen: "
                f"{', '.join(missing)}.\n"
                "Kein PDF ausgeliefert — der Markdown-CV bleibt das gültige Artefakt.\n"
                "Prüfe die Überschriften-Ebenen in "
                f"04_final_{lang}.md (Abschnitte müssen `## ` sein).\n",
                encoding="utf-8",
            )
            log.warning("pdf_renderer.missing_sections", lang=lang, sections=missing)
            write_run_log_entry(
                ctx.run_dir,
                "pdf_renderer",
                f"{lang}: Abschnitte fehlen im Template-Markdown ({', '.join(missing)}), kein PDF",
            )
            continue
