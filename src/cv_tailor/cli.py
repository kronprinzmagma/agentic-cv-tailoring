...
@cli.command(name="sync-portfolio")
@click.option(
    "--index",
    "index_path",
    type=click.Path(path_type=Path),
...
@click.option(
    ...
    type=click.Path(path_type=Path),
    default=None,
    help="Portfolio Markdown (default: sources.portfolio from config.yaml).",
)
@click.option("--dry-run", is_flag=True, default=False, help="Nur zeigen, nichts schreiben.")
def sync_portfolio_cmd(index_path: Path, portfolio_path: Path | None, dry_run: bool) -> None:
    """Sync PORTFOLIO.md into the Beleg-Index (additive, keeps existing BELG-IDs).

    The portfolio is the living record of current work — own projects, model
    benchmarks, methodical patterns. Without this the pipeline only ever sees
    the hand-copied excerpt in the Standard-CV, which drifts.
    """
    load_dotenv()
    from cv_tailor.llm import require_llm_environment
    ...
    from cv_tailor.portfolio_sync import sync_portfolio

    try:
        result = sync_portfolio(
            index_path=index_path, portfolio_path=portfolio_path, dry_run=dry_run
        )
    except FileNotFoundError as exc:
        click.echo(f"Fehler: {exc}", err=True)
        sys.exit(1)

    click.echo(f"Portfolio: {result['portfolio_path']}")
    click.echo(f"  Claims geparst:   {result['parsed']}")
    click.echo(f"  Alte entfernt:    {result['removed']}")
    if dry_run:
        click.echo(f"  (dry-run — nichts geschrieben; nächste ID BELG-{result['first_id']:03d})")
        return
    click.echo(f"  Neu hinzugefügt:  {result['added']} ab {result['first_id']}")
    click.echo(f"  Index gesamt:     {result['total']} Einträge")


@cli.command(name="refresh-index")
@click.option(
    "--in",
    "in_path",
    type=click.Path(path_type=Path),
    default=Path("data/beleg_index.json"),
    show_default=True,
    help="Input beleg_index.json to refresh.",
)
@click.option(
    "--cv",
    "cv_path",
    ...
    default=Path("data/standard_cv.md"),
    show_default=True,
    help="Current Standard-CV (Markdown) — verbatim source of truth.",
)
@click.option(
    "--out",
    "out_path",
    ...
    default=Path("data/beleg_index.json"),
    show_default=True,
    help="Output path (defaults to in-place overwrite).",
)
@click.option(
    "--dry-run",
    is_flag=True,
    default=False,
    help="Print a summary of what would change and write nothing.",
)
def refresh_index(in_path: Path, cv_path: Path, out_path: Path, dry_run: bool) -> None:
    """Deterministically refresh data/beleg_index.json against the current Standard-CV.

    LLM-free targeted transform: corrects drifted quelle_position for verbatim
    standard_cv entries, fixes BELG-040's dead tools list, adds the lost
    Software-&-Tools claims, and flags the 26 paraphrased Zusatzkontext entries
    with verbatim:false. Zeugnis entries are left byte-unchanged.
    """
    # Local imports (consistent with bootstrap) — no LLM env required.
    from cv_tailor.beleg_index import write_beleg_index
    try:
        from cv_tailor.beleg_index_refresh import (
            PARAPHRASED_IDS,
            is_verbatim,
            refresh_and_annotate,
        )
    except ImportError:
        # beleg_index_refresh trägt hartkodierte private Ingestion-Daten und
        # wird bewusst nicht in den Public-Showcase gesynct.
        click.echo(
            "Fehler: refresh-index ist im Public-Showcase nicht verfügbar "
            "(privates Ingestion-Modul).",
            err=True,
        )
        sys.exit(1)

    if not in_path.exists():
        click.echo(f"Fehler: Eingabe-Index nicht gefunden: {in_path}", err=True)
        sys.exit(1)
    if not cv_path.exists():
        click.echo(f"Fehler: Standard-CV nicht gefunden: {cv_path}", err=True)
        sys.exit(1)

    before = json.loads(in_path.read_text(encoding="utf-8"))
    after = refresh_and_annotate(before, cv_path)

    before_entries = before.get("entries", [])
    after_entries = after.get("entries", [])
    before_ids = {e["id"] for e in before_entries}
    after_ids = {e["id"] for e in after_entries}
    new_ids = sorted(after_ids - before_ids)

    before_pos = {e["id"]: e.get("quelle_position") for e in before_entries}
    positions_corrected = sum(
        1
        for e in after_entries
        if e["id"] in before_pos and e.get("quelle_position") != before_pos[e["id"]]
    )
    verbatim_false = sum(1 for e in after_entries if not is_verbatim(e))

    click.echo(f"Einträge:           {len(before_entries)} → {len(after_entries)}")
    click.echo(f"Positionen korr.:   {positions_corrected}")
    click.echo(f"verbatim:false:     {verbatim_false} (erwartet {len(PARAPHRASED_IDS)})")
    click.echo(f"Neue Tools-Einträge: {len(new_ids)} {new_ids}")
    upd = after.get("updated_stale_ids") or []
    if upd:
        click.echo(f"Aktualisiert (Zeile geändert):        {len(upd)} {upd}")
    stale = after.get("dropped_stale_ids") or []
    if stale:
        click.echo(
            f"Verworfen (nicht mehr im Standard-CV): {len(stale)} {stale}"
        )

    if dry_run:
        click.echo("[--dry-run] Es wurde nichts geschrieben.")
        return

    write_beleg_index(after, out_path)
    click.echo(f"Beleg-Index aktualisiert: {out_path}")
...
def run(stellenanzeige: Path) -> None:
    ...
    try:
        ...
        from cv_tailor.pipeline_stages import run_page_trim_if_needed
        run_page_trim_if_needed(ctx, progress_cb=click.echo)
        ...
        run_postprocess_stages(ctx, progress_cb=click.echo)
        ...
        redundancy = ctx.run_dir / "_redundancy.md"
        if redundancy.exists():
            click.echo("")
            click.echo("─" * 60)
            click.echo("Wortgleiche Wiederholungen zwischen Abschnitten gefunden:")
            click.echo(f"  {redundancy}")
            click.echo("─" * 60)

        idiom = ctx.run_dir / "_idiom.md"
        if idiom.exists():
            click.echo("")
            click.echo("─" * 60)
            click.echo("Englisch-Hinweise (mögliche Germanismen) — vor dem Versenden prüfen:")
            click.echo(f"  {idiom}")
            click.echo("─" * 60)

        translation_check = ctx.run_dir / "_translation_check.md"
        if translation_check.exists():
            click.echo("")
            click.echo("─" * 60)
            click.echo("Übersetzungs-Endprüfung hat Befunde — vor dem Versenden prüfen:")
            click.echo(f"  {translation_check}")
            click.echo("─" * 60)

        fc_open = ctx.run_dir / "_factcheck_open_questions.md"
        if fc_open.exists():
            click.echo("")
            click.echo("─" * 60)
            click.echo("Beratende Factcheck-Hinweise (haben nicht blockiert):")
            click.echo(f"  {fc_open}")
            click.echo("─" * 60)
...
def continue_() -> None:
    ...
    # Profile-Fit-Gate auch beim Wiederaufnehmen erzwingen: `continue` lief
    # bisher ohne die Gate-Entscheidung aus `run` — ein am Gate abgebrochener
    # oder davor gecrashter Lauf konnte hier ungefragt in den teuren
    # Writer-Loop laufen. Bereits explizit bestätigte Läufe (Run-Log-Eintrag)
    # werden nicht erneut gefragt.
    from cv_tailor.orchestrator import write_run_log_entry
    from cv_tailor.profile_fit import (
        check_profile_fit,
        fit_confirmed_in_log,
        format_gaps_for_cli,
        has_critical_gaps,
    )
    fit_gaps = check_profile_fit(paused_dir / "01_analyse.md")
    run_log_path = paused_dir / "_run.log"
    run_log_text = run_log_path.read_text(encoding="utf-8") if run_log_path.exists() else ""
    if fit_gaps and has_critical_gaps(fit_gaps) and not fit_confirmed_in_log(run_log_text):
        click.echo("")
        click.echo(format_gaps_for_cli(fit_gaps))
        if not click.confirm("Trotzdem fortfahren?", default=False):
            click.echo("Abgebrochen — keine Token verbraucht für Writer-Loop.")
            write_run_log_entry(paused_dir, "profile_fit", "Continue vom User wegen Profile-Fit-Gaps abgebrochen")
            sys.exit(0)
        write_run_log_entry(
            paused_dir,
            "profile_fit",
            f"User bestätigt beim Continue trotz {len(fit_gaps)} Profile-Fit-Hinweis(en)",
        )
    ...
    try:
        ...
        if klaerung_path.exists() and not answers_path.exists():
            ...
            # Antworten, die direkt in 02_antworten.md editiert wurden (statt
            # über den interaktiven Prompt oder die Web-UI), gingen bisher am
            # Klärungs-Store vorbei. save_run_clarification ist idempotent —
            # bereits importierte run_ids werden übersprungen.
            if klaerung_path.exists() and answers_path.exists():
                from cv_tailor.clarifications import save_run_clarification
                if save_run_clarification(paused_dir):
                    click.echo("  Klärungsantworten in data/clarifications.json übernommen.")
