...
def init_run(stellenanzeige_path: Path, runs_root: Path = Path("runs")) -> RunContext:
    ...
    # Optional companion research next to the posting (recherche_*/personen_*):
    # only their `## Für cv-tailor` blocks land in the run, as 00b_kontext.md.
    from cv_tailor.companion_docs import write_companion_context
    companion = write_companion_context(run_dir, stellenanzeige_path)
    ...
    if companion is not None:
        log_content += f"- [{ts}] orchestrator: Begleitrecherche übernommen ({companion.name})\n"
