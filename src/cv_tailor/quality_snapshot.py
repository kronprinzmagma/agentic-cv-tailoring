...
_NOMINALISATION = re.compile(
    r"\b\w{4,}(?:tion|sion|ment|ance|ence|ity|ness|ship)\b", re.IGNORECASE
)


def _nominalisation_density_per_100_words(final_en_text: str) -> float:
    """Per-100-word frequency of abstract nouns in the English CV.

    German CV prose is nominal ("Strukturierte Erhebung domänenspezifischer
    Anforderungen und Übersetzung in ..."); English prefers verbs ("Gathered
    requirements and turned them into ..."). A translator carrying the German
    structure over produces correct but unmistakably translated English, and
    the phrase-pattern list in `idiom_check` cannot see it — a nominal style
    is a property of the sentence, not a phrase someone can enumerate.

    English-only: German legitimately scores high on these suffixes, so a
    DE-only run returns 0.0 rather than a number that means nothing.
    """
    if not final_en_text:
        return 0.0
    wc = _count_words(final_en_text)
    if wc == 0:
        return 0.0
    return round(100 * len(_NOMINALISATION.findall(final_en_text)) / wc, 2)
...
def _bullet_length_stats(final_cv_text: str) -> dict[str, float]:
    ...
    for line in final_cv_text.splitlines():
        s = line.strip()
        if s.startswith("## "):
            in_berufserfahrung = "berufserfahrung" in s.lower()
            continue
        if s.startswith(("- ", "* ", "• ")):
            wc = _count_words(s[2:])
            if wc > 0:
                bullets.append(wc)
            continue
        # Paragraph-style Berufserfahrungs-Eintraege: the writer switched
        # from `- `-bullets to one paragraph per line under each station
        # header -- the metric silently dropped to count=0 on every new
        # run. Inside the Berufserfahrung section, each non-header prose
        # line counts as one bullet.
        if in_berufserfahrung and s and not s.startswith("#") and not s.startswith("---"):
            wc = _count_words(s)
            if wc > 0:
                bullets.append(wc)
    if not bullets:
...
def capture_run_snapshot(
    ...
    snapshot = {
        ...
        "nominalisation_density_per_100_words":
            _nominalisation_density_per_100_words(final_en),
...
_REGRESSION_METRICS_HIGHER_IS_WORSE = {
    ...
    # Nominalstil im EN-CV: steigt, wenn der Translator deutsche
    # Satzstruktur mituebersetzt. Enger Schwellwert, weil die Metrik
    # ueber Runs hinweg stabil ist.
    "nominalisation_density_per_100_words": 0.3,
}
...
def format_trend_table(snapshots: list[dict[str, Any]]) -> str:
    ...
    headers = [
        ...
        ("nom/100w", lambda s: f"{(s.get('nominalisation_density_per_100_words') or 0):.1f}"),
