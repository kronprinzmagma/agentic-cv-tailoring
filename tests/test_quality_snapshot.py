...
# --- Nominalstil-Dichte (ergänzt 2026-08-14) --------------------------------


def test_nominalisation_density_separates_nominal_from_verbal_english():
    """Nominal German-style English scores higher than idiomatic English."""
    from cv_tailor.quality_snapshot import _nominalisation_density_per_100_words as n

    nominal = (
        "Structured elicitation of domain requirements from expert user "
        "groups and translation into actionable specifications and "
        "documentation for the implementation."
    )
    verbal = (
        "Gathered requirements from specialists who don't speak engineering "
        "and turned them into specs the developers could build from."
    )
    assert n(nominal) > n(verbal)


def test_nominalisation_density_is_english_only():
    """A DE-only run returns 0.0 — German scores high on these suffixes."""
    from cv_tailor.quality_snapshot import _nominalisation_density_per_100_words as n

    assert n("") == 0.0


def test_nominalisation_density_is_a_tracked_regression_metric():
    from cv_tailor.quality_snapshot import _REGRESSION_METRICS_HIGHER_IS_WORSE

    assert "nominalisation_density_per_100_words" in _REGRESSION_METRICS_HIGHER_IS_WORSE
