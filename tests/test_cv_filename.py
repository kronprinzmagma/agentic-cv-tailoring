...
from cv_tailor.cv_filename import friendly_cv_filename, _extract_company
...
def test_company_extraction_stops_at_parenthesis():
    """Klammer-Inhalte sind Quellenhinweise, kein Teil des Firmennamens.

    Regression (2026-09-07): "**Arbeitgeber:** Sunrise GmbH (in den
    Seiten-Metadaten der Anzeige: Sunrise Communications AG)" ergab
    "Sunrise GmbH in den" — im Dateinamen, den der Recruiter sieht.
    """
    assert _extract_company(
        "**Arbeitgeber:** Sunrise GmbH (in den Seiten-Metadaten der Anzeige: "
        "Sunrise Communications AG)"
    ) == "Sunrise"
    assert _extract_company("**Arbeitgeber:** insurtechco AG") == "insurtechco"
    assert _extract_company("**Arbeitgeber:** HealthApp AG, Winterthur") == "HealthApp"


POSTING_COACH_FORM = """# Stellenanzeige — iWay AG: Product Owner (80–100 %)

**Quelle:** https://jobs.iway.ch/...
**Firma:** iWay AG, KMU mit über 70 Mitarbeitenden, gegründet 1995
**Ort:** Zürich
"""


def test_coach_heading_form_yields_company_and_position():
    """Jobcoach convention `# Stellenanzeige — Firma: Position` + `**Firma:**`.

    Regression (2026-09-18): three runs in a row produced
    "CV_… - Stellenanzeige Swisscom.md" and were renamed by hand.
    """
    fn = friendly_cv_filename(POSTING_COACH_FORM, language="de")
    assert fn == "CV_Alex_Müller-000000 - iWay - Product Owner.md" or (
        "iWay" in fn and "Product Owner" in fn and "Stellenanzeige" not in fn
    )


def test_coach_heading_company_fallback_without_firma_line():
    posting = "# Stellenanzeige — Swisscom (Schweiz) AG: Senior Product Manager\n\ntext\n"
    assert _extract_company(posting) == "Swisscom"
    fn = friendly_cv_filename(posting)
    assert "Senior Product Manager" in fn and "Stellenanzeige" not in fn
