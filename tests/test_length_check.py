...
from cv_tailor.length_check import (
    BULLET_MAX_WORDS,
    KOMPETENZEN_MAX_COUNT,
    SUMMARY_MAX_WORDS,
    SUMMARY_MIN_WORDS,
    check_berufserfahrung_bullet_lengths,
    check_kompetenzen_count,
...
def test_summary_too_short_vetos():
    issues = check_summary_length(SHORT_SUMMARY)
    assert len(issues) == 1
    assert issues[0].kind == "summary_too_short"
    assert "120–160" in issues[0].detail


def test_summary_at_min_passes():
    txt = "Wort " * SUMMARY_MIN_WORDS
...
# ── Schlüsselkompetenzen count check ────────────────────────────────────────

def _kompetenzen_draft(n: int) -> str:
    return "\n\n".join(
        f"**Kompetenz {i}** - Beschrieb-Satz für Punkt {i}." for i in range(1, n + 1)
    )


def test_kompetenzen_at_max_passes():
    assert check_kompetenzen_count(_kompetenzen_draft(KOMPETENZEN_MAX_COUNT)) == []


def test_kompetenzen_over_max_vetos():
    issues = check_kompetenzen_count(_kompetenzen_draft(KOMPETENZEN_MAX_COUNT + 1))
    assert len(issues) == 1
    assert issues[0].kind == "kompetenzen_too_many"
    assert "Streiche" in issues[0].detail


def test_kompetenzen_ignores_non_headline_lines():
    """Plain prose and bold mid-sentence shouldn't count as entries."""
    draft = _kompetenzen_draft(3) + "\n\nEin Satz mit **Bold** mittendrin zählt nicht als Punkt."
    assert check_kompetenzen_count(draft) == []
...
def test_check_section_length_dispatches_correctly():
    ...
    assert check_section_length("schluesselkompetenzen", _kompetenzen_draft(8))
    assert not check_section_length("schluesselkompetenzen", _kompetenzen_draft(5))
